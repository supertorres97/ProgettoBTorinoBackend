...
public class OrdineImpl implements OrdineService {
	...
	public OrdineImpl(OrdineRepository ordR, UtenteRepository utnR, Logger log, MessaggioService msgS) {
		this.log = log;
		this.ordR = ordR;
		this.utnR = utnR;
		this.msgS = msgS;
	...
	public void create(OrdineReq req) throws Exception {
		...
		if (ord.isPresent()) {
			throw new Exception(msgS.getMessaggio("ordine-presente"));
		}
		if (utn.isEmpty()) {
			throw new Exception(msgS.getMessaggio("ordine-assente"));
		}
	...
	public void update(OrdineReq req) throws Exception {
		...
		try {
			...
			throw new Exception(msgS.getMessaggio("invalid-status") + req.getStatus());
		}
	...
	public void logicalDelete(Integer id) throws Exception {
		...
		if (o.getStatus() == Status.Annullato) {
			throw new Exception(msgS.getMessaggio("ordine-gia-eliminato"));
		}
